#!/usr/bin/env python3
"""
Intelligent Content Analyzer with Duplicate Removal
Analyzes ALL content, removes duplicates, and creates intelligent categories
"""
...
class IntelligentContentAnalyzerWithDedup:
    def __init__(self):
    ...
    def create_intelligent_categories(self):
        """Create intelligent medical categories with duplicate removal"""
        print("🧠 Creating intelligent medical categories...")
        ...
        # Remove duplicates and track used items
        critical_care_items = list(set(critical_care_items))
        used_items.update(critical_care_items)
            ...
            'collections': [
                {
                    ...
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in critical_care_items if 'ecmo' in self.content_analysis[item_id]['title'].lower()])
                },
                {
                    'id': 'emergency-procedures',
                    'title': 'Emergency Procedures',
                    'description': 'Emergency trauma surgery and life-saving procedures',
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in critical_care_items if any(term in self.content_analysis[item_id]['title'].lower() for term in ['emergency', 'trauma', 'cpr'])])
                },
                {
                    'id': 'vascular-access',
                    'title': 'Vascular Access',
                    'description': 'Central line placement and vascular procedures',
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in critical_care_items if any(term in self.content_analysis[item_id]['title'].lower() for term in ['vascular', 'central line', 'cannulation'])])
                },
                {
                    'id': 'critical-care-general',
                    'title': 'Critical Care General',
                    'description': 'Other critical care procedures and protocols',
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in critical_care_items if not any(term in self.content_analysis[item_id]['title'].lower() for term in ['ecmo', 'emergency', 'trauma', 'cpr', 'vascular', 'central line', 'cannulation'])])
                }
            ]
        })
        ...
        # Remove items already used and duplicates
        surgical_items = [item for item in set(surgical_items) if item not in used_items]
        used_items.update(surgical_items)
            ...
            'collections': [
                {
                    ...
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in surgical_items if any(term in self.content_analysis[item_id]['title'].lower() for term in ['appendectomy', 'cholecystectomy', 'hernia'])])
                },
                {
                    'id': 'minimally-invasive',
                    'title': 'Minimally Invasive Surgery',
                    'description': 'Laparoscopic and thoracoscopic procedures',
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in surgical_items if any(term in self.content_analysis[item_id]['title'].lower() for term in ['laparoscopic', 'thoracoscopic', 'minimally invasive'])])
                },
                {
                    'id': 'specialized-surgery',
                    'title': 'Specialized Surgery',
                    'description': 'Complex surgical procedures',
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in surgical_items if not any(term in self.content_analysis[item_id]['title'].lower() for term in ['appendectomy', 'cholecystectomy', 'hernia', 'laparoscopic', 'thoracoscopic', 'minimally invasive'])])
                }
            ]
        })
        ...
        # Remove items already used and duplicates
        neonatal_items = [item for item in set(neonatal_items) if item not in used_items]
        used_items.update(neonatal_items)
            ...
            'collections': [
                {
                    ...
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in neonatal_items if 'neonatal' in self.content_analysis[item_id]['title'].lower()])
                },
                {
                    'id': 'pediatric-surgery',
                    'title': 'Pediatric Surgery',
                    'description': 'Surgical procedures for children',
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in neonatal_items if 'pediatric' in self.content_analysis[item_id]['title'].lower()])
                },
                {
                    'id': 'feeding-nutrition',
                    'title': 'Feeding & Nutrition',
                    'description': 'Gastrostomy and feeding procedures',
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in neonatal_items if any(term in self.content_analysis[item_id]['title'].lower() for term in ['feeding', 'gastrostomy', 'nutrition', 'g-tube'])])
                },
                {
                    'id': 'neonatal-pediatric-general',
                    'title': 'Neonatal & Pediatric General',
                    'description': 'Other neonatal and pediatric procedures',
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in neonatal_items if not any(term in self.content_analysis[item_id]['title'].lower() for term in ['neonatal', 'pediatric', 'feeding', 'gastrostomy', 'nutrition', 'g-tube'])])
                }
            ]
        })
        ...
        # Remove items already used and duplicates
        guideline_items = [item for item in set(guideline_items) if item not in used_items]
        used_items.update(guideline_items)
            ...
            'collections': [
                {
                    ...
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in guideline_items if any(term in self.content_analysis[item_id]['title'].lower() for term in ['ecmo', 'sepsis', 'critical', 'cpr'])])
                },
                {
                    'id': 'surgical-guidelines',
                    'title': 'Surgical Guidelines',
                    'description': 'Surgical protocols and procedures',
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in guideline_items if any(term in self.content_analysis[item_id]['title'].lower() for term in ['surgical', 'surgery', 'appendectomy', 'antibiotic'])])
                },
                {
                    'id': 'neonatal-guidelines',
                    'title': 'Neonatal Guidelines',
                    'description': 'Neonatal and pediatric care protocols',
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in guideline_items if any(term in self.content_analysis[item_id]['title'].lower() for term in ['neonatal', 'nicu', 'pediatric', 'dosing'])])
                },
                {
                    'id': 'general-guidelines',
                    'title': 'General Guidelines',
                    'description': 'Other clinical guidelines and protocols',
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in guideline_items if not any(term in self.content_analysis[item_id]['title'].lower() for term in ['ecmo', 'sepsis', 'critical', 'cpr', 'surgical', 'surgery', 'appendectomy', 'antibiotic', 'neonatal', 'nicu', 'pediatric', 'dosing'])])
                }
            ]
        })
        ...
        # Remove items already used and duplicates
        education_items = [item for item in set(education_items) if item not in used_items]
        used_items.update(education_items)
            ...
            'collections': [
                {
                    ...
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in education_items if any(term in self.content_analysis[item_id]['title'].lower() for term in ['resident', 'curriculum', 'training'])])
                },
                {
                    'id': 'fellowship-program',
                    'title': 'Fellowship Program',
                    'description': 'Fellowship training and case requirements',
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in education_items if any(term in self.content_analysis[item_id]['title'].lower() for term in ['fellow', 'fellowship', 'case', 'requirements'])])
                },
                {
                    'id': 'competency-assessment',
                    'title': 'Competency Assessment',
                    'description': 'Goals, objectives, and competency tracking',
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in education_items if any(term in self.content_analysis[item_id]['title'].lower() for term in ['goals', 'objectives', 'competencies', 'tracked'])])
                },
                {
                    'id': 'general-education',
                    'title': 'General Education',
                    'description': 'Other educational materials and documents',
                    'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in education_items if not any(term in self.content_analysis[item_id]['title'].lower() for term in ['resident', 'curriculum', 'training', 'fellow', 'fellowship', 'case', 'requirements', 'goals', 'objectives', 'competencies', 'tracked'])])
                }
            ]
        })
                ...
                'collections': [
                    {
                        ...
                        'items': self.remove_duplicates([self._create_content_item(item_id) for item_id in remaining_items])
                    }
                ]
            })
    ...
    def generate_intelligent_structure(self):
        """Generate the complete intelligent structure with duplicate removal"""
        print("🚀 Starting Intelligent Content Analysis with Duplicate Removal...")
        
        self.load_transcription_data()
        self.analyze_content_patterns()
        space_cards = self.create_intelligent_categories()
        
        # Recalculate counts after duplicate removal
        total_content = 0
        ...
        structure = {
            ...
            'analysis_summary': {
                ...
                'duplicates_removed': len(self.content_analysis) - total_content,
                'categories_created': len(space_cards),
                'collections_created': sum(len(sc['collections']) for sc in space_cards)
            }
        }
        
        print(f"✅ Generated intelligent structure with {total_content} unique items")
        print(f"📊 Removed {len(self.content_analysis) - total_content} duplicates")
        
        return structure
    
    def save_structure(self, structure):
        """Save the intelligent structure"""
...
if __name__ == "__main__":
    analyzer = IntelligentContentAnalyzerWithDedup()
    structure = analyzer.generate_intelligent_structure()
    analyzer.save_structure(structure)
    print("🎉 Intelligent content analysis with duplicate removal complete!")
